...
interface HttpClientInterface
{
     ...
     *
     * @return ResponseInterface         HTTP response as received from the remote
     *                                   server.
     * @throws ResponseErrorException    If the remote server returned a response.
     *                                   with a non-successful status code (non-2xx).
     * @throws SslCertificateException   If the SSL certificate of the remote host is
     *                                   invalid or broken or otherwise prevents the
     *                                   request from being carried out. Note that
     *                                   the verification of SSL certificates can be
     *                                   disabled on the client.
     * @throws UnresolvableHostException If the hostname cannot be resolved to a
     *                                   valid IP address.
     * @throws ConnectionException       If a connection to the remote server cannot
     *                                   be established or fails due to communication
     *                                   issues. This might also be a local problem.
     * @throws WanderException           If an unexpected error occurs during the
     *                                   request. This hints at a bug in the library.
     */
